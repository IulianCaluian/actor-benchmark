...
namespace ConsoleActorBenchmark.Actors
{
    internal class Account : IActor
    {
        ...
        public Task ReceiveAsync(IContext context)
        {
            ...
            {
                case Credit(_, var replyTo) when AlreadyProcessed(replyTo):
                    return Reply(replyTo);
                case Credit(var amount, var replyTo):
                    return AdjustBalance(context, replyTo, amount);
                case Debit(_, var replyTo) when AlreadyProcessed(replyTo):
                    return Reply(replyTo);
                case Debit(var amount, var replyTo) when amount + _balance >= 0:
                    return AdjustBalance(context, replyTo, amount);
                case Debit msg:
